        ...
        if let Some(kv) = kv {
            ...
            while insert_pos < self.maybe_next_key_values.len()
                ...
                && this_key_prefix_to_map
                    >= self.ordered_key_prefixes_to_map[insert_pos]
            {
                insert_pos += 1;
            }
            ...
            for i in 1..insert_pos {
                ...
                self.ordered_key_prefixes_to_map[i - 1] =
                    self.ordered_key_prefixes_to_map[i];
                self.maybe_next_key_values[i - 1] =
                    self.maybe_next_key_values[i].take();
            }

            // Insert new values at insert_pos-1
            let final_pos = insert_pos - 1;
            self.ordered_iter_ids[final_pos] = iter_id;
            self.ordered_key_prefixes_to_map[final_pos] =
                this_key_prefix_to_map;
