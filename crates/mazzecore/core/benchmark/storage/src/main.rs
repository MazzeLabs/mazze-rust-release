...
impl TxReplayer {
    ...
    pub fn new(
        mazze_data_dir: &str, reset_db: bool, debug_snapshot_integrity: bool,
    ) -> errors::Result<TxReplayer> {
        if reset_db {
            match fs::remove_dir_all(mazze_data_dir) {
                Ok(_) => {}
        ...
        let commit_log_path = mazze_data_dir.to_string() + "/commit_log";
    ...
    pub fn commit(
        ...
        latest_state.commit(epoch_id, None).unwrap();
        {
            ...
            state_availability_boundary_mut.main_chain.push(epoch_id);
        }
        ...
        if block_height > confirmation_lag {
            ...
            self.storage_manager
                .get_storage_manager()
                .maintain_snapshots_main_chain_confirmed(
                    confirmed_height,
        ...
        if let Some(sender) = tx.sender {
            let maybe_account = latest_state
                .get_account(
                    ...
                    // because Mazze use a newer version
                ...
                Some(mut account) => {
                    account.balance = account
                        ...
                        .overflowing_sub(
                            ...
                            // ethereum-types because Mazze use a newer
                            // version
                            unsafe {
                                std::mem::transmute(
...
fn tx_replay(matches: ArgMatches) -> errors::Result<()> {
    let tx_replayer = TxReplayer::new(
        matches.value_of("mazze_data_dir").unwrap(),
...
fn main() -> errors::Result<()> {
    ...
    let matches = App::new("Mazze storage benchmark")
        .arg(
        ...
        .arg(
            Arg::with_name("mazze_data_dir")
                .value_name("Mazze data dir")
                .help("Mazze data dir")
                .short("d")
                .takes_value(true),
        )
