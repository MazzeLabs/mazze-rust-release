...
use dag::{get_future, topological_sort, Graph, RichDAG, RichDETS, DAG, DETS};
...
pub struct ConsensusGraphInner {
    ...
    /// The set of *graph* tips in the DAG-Embedded Tree Structure (DETS) for
    /// mining. Note that this set does not include non-active partial
    /// invalid blocks
...
impl ConsensusGraphInner {
    ...
    pub fn get_timer_chain_hashes(&self) -> Vec<H256> {
        self.timer_chain
            .iter()
            .map(|&arena_index| self.arena[arena_index].hash)
            .collect()
    ...
    pub fn get_timer_chain_difficulty(&self) -> U256 {
        U256::from(self.best_timer_chain_difficulty.max(0) as u128)
    }
    ...
    fn get_deferred_state_arena_index(
        ...
        if main_idx < self.main_chain.len() && self.main_chain[main_idx] == me {
            return Ok(
    ...
    pub fn main_block_processed(&self, main_hash: &H256) -> bool {
        if let Some(height) = self.data_man.block_height_by_hash(main_hash) {
            if let Ok(epoch_hash) = self.get_main_hash_from_epoch_number(height)
            {
...
impl StateMaintenanceTrait for ConsensusGraphInner {
    fn get_main_hash_from_epoch_number(
        ...
        ConsensusGraphInner::get_main_hash_from_epoch_number(self, epoch_number)
    }
