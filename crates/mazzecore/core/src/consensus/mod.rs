...
///
/// ConsensusGraph maintains the DAG-Embedded Tree Structure (DETS) structure of
/// the client and implements *Timer Chain WLSR*/*Mazze* algorithm to determine
/// the block total order. It dispatches transactions in epochs to
/// ConsensusExecutor to process. To avoid executing too many execution reroll
/// caused by transaction order oscillation. It defers the transaction execution
/// for a few epochs.
///
...
impl ConsensusGraph {
            ...
            for hash in hashes {
                ...
                for tx in block.transactions.iter() {
                    if space == Space::Native && tx.space() != Space::Native {
                        // For mazze_gasPrice, we only count Native
                        // transactions.
                        continue;
                    }
    ...
    > {
        if filter.space == Space::Ethereum {
            let phantom_block = self
                ...
                .ok_or(FilterError::UnknownBlock { hash: assumed_main })?;
    ...
    pub fn filter_block_traces(
        ...
        for (main_hash, block_hash, block_trace, block_txs) in block_traces {
            ...
            let epoch_number = self
                ...
                .ok_or_else(|| {
                FilterError::Custom(
                    format!("main block header missing, hash={:?}", main_hash)
                        .into(),
                )
            })?;
                    ...
                    .transactions_traces_by_block_hash(&b.hash())
                {
                    ...
                    Some((main_hash, block_traces)) => {
                        ...
                        if main_hash != main.hash() {
                            return Err(
                                "Inconsistent state: main hash mismatch".into(),
                            );
                        }
    ...
    fn get_state_db_by_epoch_number_with_space(
        ...
        let hash = self.inner.read().get_main_hash_from_epoch_number(height)?;
        Ok(StateDb::new(
