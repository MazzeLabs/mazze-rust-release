        ...
        if view_in_term == 0 {
            let (validator_verifier, vrf_seed) =
                state_view.pos_state().get_committee_at(term).map_err(|e| {
                    ...
                    VMStatus::Error(StatusCode::MAZZE_INVALID_TX)
                })?;
            let epoch = term + 1;
    ...
    fn process_user_transaction(
        ...
        let events = match tx.payload() {
            ...
            TransactionPayload::MainDecision(main_decision) => {
                main_decision.execute(state_view, tx, spec)?
            }
            ...
            _ => return Err(VMStatus::Error(StatusCode::MAZZE_UNEXPECTED_TX)),
        };
...
impl ExecutableBuiltinTx for MainBlockDecision {
    fn execute(
        &self, state_view: &dyn StateView, tx: &SignatureCheckedTransaction,
        spec: &Spec,
    ) -> Result<Vec<ContractEvent>, VMStatus> {
        if !spec.catch_up_mode {
            ...
            let signature = match authenticator {
                ...
                _ => Err(VMStatus::Error(StatusCode::MAZZE_INVALID_TX)),
            }?;
            state_view
                .pos_state()
                .validate_main_decision(self, signature)
                .map_err(|e| {
                    diem_error!("main decision tx error: {:?}", e);
                    VMStatus::Error(StatusCode::MAZZE_INVALID_TX)
                })?;
        }
        Ok(vec![self.to_event()])
...
impl ExecutableBuiltinTx for DisputePayload {
    fn execute(
        ...
        if !verify_dispute(self) {
            return Err(VMStatus::Error(StatusCode::MAZZE_INVALID_TX));
        }
        Ok(vec![self.to_event()])
    }
}
