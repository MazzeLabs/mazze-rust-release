...
impl BLSPrivateKey {
    ///
    pub fn raw_key(self) -> RawPrivateKey {
        self.0
    }
}
...
impl From<BLSPublicKeyUnchecked> for BLSPublicKey {
    fn from(unchecked: BLSPublicKeyUnchecked) -> Self {
        Self(unchecked.0)
    }
}

impl From<BLSSignatureUnchecked> for BLSSignature {
    fn from(unchecked: BLSSignatureUnchecked) -> Self {
        Self(unchecked.0)
    }
}
