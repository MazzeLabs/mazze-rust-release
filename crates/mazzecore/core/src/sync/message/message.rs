...
use crate::{
    ...
    sync::{
        message::throttling::Throttle, Error, SYNC_PROTO_V1,
    },
};
...
build_msg_impl! { StatusV3, msgid::STATUS_V3, "StatusV3", SYNC_PROTO_V1, SYNC_PROTO_V1 }
build_msg_impl! { Heartbeat, msgid::HEARTBEAT, "Heartbeat", SYNC_PROTO_V1, SYNC_PROTO_V1 }
...
mark_msg_version_bound!(GetBlockHashesResponse, SYNC_PROTO_V1, SYNC_PROTO_V1);
...
// (Pre-launch consolidation removed the per-msgid version-cascade
// helpers — SnapshotManifestResponse, GetBlockHashesResponse, and
// SnapshotChunkResponse now have exactly one wire schema each, so the
// generic handle_message<> path is enough.)
