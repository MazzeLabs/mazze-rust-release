...
# Phase 5a: shadow-mirror flags removed. DBManager is now MDBX-native
# primary — no dual-write, no `enable_mdbx_shadow_*` config keys.
# state_db_type = "mdbx" (in hydra.toml) is required for boot.
has_chain_data() {
  local base="$1"
